...
void dijkstra(vector<vector<Edge> > &graph, int source, vector<int> &dist, vector<int> &parent)
{
    int n = graph.size();
    dist.assign(n, INF);
    parent.assign(n, -1);
...
void printShortestPath(vector<int> &parent, int vertex)
{
    if (vertex == -1)
    {
        return;
...
int main()
{
    ...
    for (int i = 0; i < m; ++i)
    {
        ...
        graph[v].emplace_back(u, w); // For undirected graphs
    }
